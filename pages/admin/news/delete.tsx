...
Index.getInitialProps = async (ctx: DocumentContext) => {
  ...
  try {
    allNews = await adminReqService.showAllDeleted();
  } catch (error) {
    err = error;
  }
  return {
...
export default function Index({ props }) {
  ...
  function restoreNews(row) {
    adminReqService
      .postRestoreNews(row.id)
      .then((res) => {
        let arrTemp = [...listNews];
        const filteredItems = arrTemp.filter((item) => item !== row);
        ...
        console.log(filteredItems);
        setlistNews(filteredItems);
        toast.notify(`Đã khôi phục bài viết ID: ${row.id}`, {
          title: "Thành công",
          duration: 5,
          type: "success",
        });
  ...
  function submitDelete(row) {
    confirmAlert({
      ...
      message: `Bạn có chắc muốn khôi phục bài viết có ID: ${row.id}`,
      buttons: [
        {
          label: "Đồng ý",
          onClick: () => restoreNews(row),
        },
        {
          label: "Không",
          onClick: () => {},
  ...
  function renderBadgesStatus(cell, row) {
    return (
      <span className="badge badge-success" onClick={() => submitDelete(row)}>
        Khôi Phục
      </span>
    );
  }
